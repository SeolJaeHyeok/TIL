"""
문제
어떤 회사에서 담당 업무에 따라 사원 몇 명을 재택근무로 전환하려 합니다. 업무는 재택 가능한 업무, 출근해야 할 업무 두 가지 종류가 있습니다.
어떤 사원의 업무가 재택 가능한 업무로만 이루어져 있다면, 그 사원은 재택근무 대상자입니다.

회사에는 1번부터 num_employees번까지 각각 다른 사원 번호를 가진 num_employees명의 사원들이 있습니다.
회사의 사원들은 num_teams개의 팀으로 나뉘어 있으며, 사원마다 소속된 팀 번호가 있습니다. 팀마다 최소 한 명은 출근해야 합니다.
만약 어떤 팀에서 팀원 모두가 재택근무 대상자일 때, 사원 번호가 가장 빠른 사람이 출근하기로 했습니다. 이에 해당하지 않는 재택근무 대상자들은 재택근무를 하게 됩니다.
당신은 재택근무를 하게 될 사원의 번호를 구하려 합니다.
...
팀의 개수를 나타내는 정수 num_teams, 재택 가능한 업무들을 나타내는 문자열 배열 remote_tasks, 출근해야 할 업무들을 나타내는 문자열 배열 office_tasks,
사원들의 정보를 1번 사원부터 순서대로 나타내는 문자열 배열 employees가 매개변수로 주어집니다.
재택근무를 하게 될 사원들의 번호를 정수 배열에 오름차순으로 담아 return 하도록 solution 함수를 완성해 주세요.
최소 한 명 이상 재택근무를 하게 될 사원이 존재하는 경우만 주어집니다.
...
Input
3 ["development","marketing","hometask"] ["recruitment","education","officetask"] ["1 development hometask","1 recruitment marketing","2 hometask","2 development marketing hometask","3 marketing","3 officetask","3 development"]
Output
[1,4,5,7]

Input
2 ["design"] ["building","supervise"] ["2 design","1 supervise building design","1 design","2 design"]
Output
[3, 4]
"""
...
def solution(num_teams, remote_tasks, office_tasks, employees):
    ...
    for i in range(len(employees)):
        task = employees[i].split(' ')
        task.insert(1, i + 1)
        table[task[0]].append(task[1:])

    for key, value in table.items():
        for v in value:
            each_task = v[1:]

            if ''.join(sorted(each_task)) in ''.join(sorted(remote_tasks)):
                answer[key].append(v[0])

    for k in answer.keys():
        if len(answer[k]) == len(table[k]):
            answer[k].pop(0)
