...
BFS 알고리즘은 너비 우선 탐색이라는 의미를 가진다. 쉽게 말해 **가까운 노드부터 탐색하는 알고리즘**이다. DFS는 최대한 멀리 있는 노드를 우선으로 탐색한는 방식으로 동작한다고 했다. BFS는 그 반대다. 그렇다면 BFS는 어떻게 구현할 수 있을까?
...
너비 우선 탐색 알고리즘인 BFS는 큐 자료구조에 기초한다는 점에서 구현이 간단하다. 실제로 구현함에 있어 앞서 언급한 대로 `deque` 라이브러리를 사용하는 것이 좋으면 탐색을 수행함에 있어 *O(N)*의 시간이 소요된다. 일반적인 경우 실제 수행 시간은 DFS보다 좋은 편이다.
...
DFS와 BFS에 대해서 간단히 정리하면 아래 표와 같다.
...
앞서 DFS와 BFS를 설명할 때 전형적인 그래프 그림을 이용했는데 1차원 배열이나 2차원 배열 또한 그래프 형태로 생각하면 수월하게 문제를 풀 수 있다.

예를 들어 게임 맵이 3 X 3 형태의 2차원 배열이고 게임 캐릭터가 (1, 1) 좌표에 있다고 표현할 때처럼 각 데이터를 좌표라고 생각해보자. 이때 각 좌표를 상하좌우로만 이동할 수 있다면 어떨까? 모든 좌표의 형태를 다음처럼 그래프의 형태로 바꿔서 생각할 수 있다.

<img src="../images/5-3.png"/>

코딩 테스트 중 2차원 배열에서의 탐색 문제를 만나면 이렇게 그래프의 형태로 바꿔서 생각하면 풀이 방법을 좀 더 쉽게 떠올릴 수 있다. 그러므로 코딩 테스트에서 탐색 문제를 보면 그래프 형태로 표현한 다음 풀이법을 고민해보도록 하자.