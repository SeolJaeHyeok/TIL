...
- **알려지지 않은 기준**을 이용한 검색에 유용, ex) 연봉이 상위 5%인 사람
- 메인 쿼리가 실행되기 이전에 **한 번만 실행**
- 한 문장에서 **여러 번 사용 가능**

```sql
-- 사원 한 명의 급여(2500)를 알고 있는 상태에서 더 높은 급여를 받는 사원을 조회
-- 서브 쿼리 사용 x
SELECT * FROM employee
WHERE salary > 2500;

-- 사원 한 명의 급여를 알지 못한 상태에서 더 높은 급여를 받는 사원을 조회
-- 서브 쿼리 사용
SELECT * FROM employee
WHERE salary > 
(SELECT salary FROM employee WHERE name='jaehyeok'); -- 서브 쿼리
```
...
- 결과가 한 행만 나오는 서브쿼리
- **서브쿼리의 결과가 1개의 값만 반환**하고 이를 메인쿼리로 전달하는 쿼리
- ex) 사원 번호가 1인 사람의 급여

#### 다중 행 서브쿼리

- **서브쿼리가 결과를 2개 이상 반환**하고 이 결과를 메인쿼리로 전달하는 쿼리
- ex) 반에서 키가 160cm인 사람
...
| :--: | :----------------------------------------: |
|  IN  |           하나라도 만족하면 반환           |
| ANY  | 하나라도 만족하면 반환<br />비교 연산 가능 |
| ALL  |   모두 만족하면 반환<br />비교 연산 가능   |

```sql
1 IN (1, 2, 3, 4) -- 1이 존재하는가? ->참
10 <ANY (1, 2, 3, 4) -- 최댓값(4)이 10보다 큰가? -> 거짓
99 >=ALL(99, 100, 101) -- 최댓값(101)이 99보다 큰가? -> 거짓
```
...
- 스칼라 서브쿼리는 오로지 한 행만 반환하고 마치 `JOIN` 을 사용한 것과 같은 결과가 나타난다.
