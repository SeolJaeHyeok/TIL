...
관계형 데이터베이스는 중복되어 있는 정보를 새로운 테이블로 만들어 기존 테이블에서 새로 만든 테이블을 참조하도록 할 수 있다.

어떻게 구현해야 하는지 순서대로 살펴보도록 하자.

### 3.1 테이블 이름 변경

<img src="./images/25.png" />

기존에 존재하던 `topic` 테이블의 이름을 `topic_backup` 테이블로 변경해 준다.

### 3.2 author 테이블 생성

<img src="./images/26.png" />

중복된 정보들을 위하 `author` 테이블을 따로 만들어 준다.
...
기존 테이블의 구조를 위와 같이 변경해 준다. 이렇게 함으로써`topic` 테이블의  `author_id` 컬럼과 `author` 테이블의 `id` 컬럼을 매핑시킬 준비가 되는 것이다.
