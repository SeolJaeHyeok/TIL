...
첫 번째 렌더링을 할 때는 renderToString 대신 renderToStaticMarkup이라는 함수를 사용했다. renderToStaticMarkup은 리액트를 사용하여 정적인 페이지를 만들 때 사용한다. 이 함수로 만든 리액터 렌더링 결과물은 클라이언트 쪽에서 HTML DOM 인터랙션을 지원하기 힘든다.
...
브라우저에서 상태를 재사용할 때는 다음과 같이 스토어 생성 과정에서 `window._ _PRELOADED_STATE __를 초깃값으로 사용하면 된다.
...
#### 20.4.9 usePreloader Hook 만들어서 사용
...
<img src="./images/20_14.png" />

## 20.6 SSR의 환경 구축을 위한 대안

서버 사이드 렌더링 자체만 놓고 보면 꽤나 간단한 작업이지만 데이터 로딩, 코드 스플리팅까지 하게 된다면 번거로운 작업이 될 수 있다.

#### 20.6.1 Next.js

[Next.js](https://nextjs.org/)라는 리액트 프레임워크를 사용하면 이 작업을 최소한의 설정으로 간단하게 처리할 수 있다. 그 대신 몇가지 제한이 있다. 가장 대표적인 것이 **리액트 라우터와 호환되지 않는 점**이다. 호환되지 않기 때문에 이미 작성된 프로젝트에 적용하는 것은 매우 까다롭다. 그리고 리액트 라우터는 컴포넌트 기반으로 라우트를 설정하는 반면에 Next.js는 파일 시스템에 기반하여 라우트를 설정한다. 컴포넌트 파일의 경로와 파일 이름을 사용하여 라우트를 설정하는 것이다.

#### 20.6.2 Razzle

[Razzle](https://github.com/jaredpalmer/razzle) 또한 Next.js처럼 SSR을 쉽게 할 수 있도록 해 주는 도구이며, 프로젝트의 구성이 CRA와 매우 유사하다는 장점이 있다. 그렇기에 프로젝트의 구조를 마음대로 설정할 수 있으며, 리액트 라우터와도 호환이 잘 된다.

## 20.7 정리

SSR은 프로젝트를 만들 때 꼭 해야 하는 작업은 아니지만 만약 사용자가 많아지거나 검색 엔진 최적화 및 UX를 향상시키길 원한다면 도입을 고려해 볼 만한 가치가 있는 기술이다. 하지만 이를 도입하게 되면 프로젝트 구조가 조금은 복잡해질 수 있는 단점 또한 존재한다.
