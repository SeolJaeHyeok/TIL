...
mapStateToProps와 mapDispatchToProps에서 반환하는 객체 내부의 값들은 타깃 컴포넌트(CounterContainer)의 props로 전달된다. mapStateToProps는 state를 파라미터로 받아오며, **이 값은 현재 스토어가 지니고 있는 상태를 가리킨다.** mapDispatchToProps의 경우 store의 내장 함수 dispatch를 파라미터로 받아온다. 현재 mapDispatchToProps에서는 진행 절차를 설명하기 위해 임시로 console.log를 사용했다.
